...
func loadFrom(loadPath string, maxSizeBytes uint64) (*Tracker, error) {
	...
	for {
		...
		key := statKey{
			...
			metricName: mt.cloneMetricNameGroupLocked([]byte(r.MetricName)),
		}
		mt.store[key] = si
...
// cloneMetricNameGroupLocked uses the same idea as strings.Clone.
...
func (mt *Tracker) cloneMetricNameGroupLocked(metricName []byte) string {
...
func (mt *Tracker) RegisterIngestRequest(accountID, projectID uint32, metricName []byte) {
	...
	si := mt.nextRecordLocked()
	sk.metricName = mt.cloneMetricNameGroupLocked(metricName)
	mt.store[sk] = si
	mt.mu.Unlock()
