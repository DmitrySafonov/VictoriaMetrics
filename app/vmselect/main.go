...
var (
	...
	metricNamesStatsResetAuthKey = flagutil.NewPassword("resetMetricsStatsAuthKey", "authKey for reseting metric names usage cache via /api/v1/admin/status/metric_names_stats/reset. It overrides -httpAuth.*")
...
var (
	...
	metricNamesStatsResetErrors   = metrics.NewCounter(`vm_http_request_errors_total{path="/v1/admin/status/metric_names_stats/reset"}`)
)
